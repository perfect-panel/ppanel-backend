...
typedef struct MigrateConfigRef {
  struct StringRef driver;
  struct StringRef dsn;
} MigrateConfigRef;

typedef struct MigrateOutcomeRef {
  uint32_t version;
  bool dirty;
...
var MigrateServiceImpl MigrateService

type MigrateService interface {
	up(cfg MigrateConfig) MigrateOutcome
	version(cfg MigrateConfig) MigrateOutcome
}

//export CMigrateService_up
func CMigrateService_up(cfg C.MigrateConfigRef, slot *C.void, cb *C.void) {
	_new_cfg := newMigrateConfig(cfg)
	resp := MigrateServiceImpl.up(_new_cfg)
	resp_ref, buffer := cvt_ref(cntMigrateOutcome, refMigrateOutcome)(&resp)
...
//export CMigrateService_version
func CMigrateService_version(cfg C.MigrateConfigRef, slot *C.void, cb *C.void) {
	_new_cfg := newMigrateConfig(cfg)
	resp := MigrateServiceImpl.version(_new_cfg)
	resp_ref, buffer := cvt_ref(cntMigrateOutcome, refMigrateOutcome)(&resp)
	asmcall.CallFuncG0P2(unsafe.Pointer(cb), unsafe.Pointer(&resp_ref), unsafe.Pointer(slot))
	runtime.KeepAlive(resp_ref)
	runtime.KeepAlive(resp)
...
func newMigrateConfig(p C.MigrateConfigRef) MigrateConfig {
	return MigrateConfig{
		driver: newString(p.driver),
		dsn:    newString(p.dsn),
	}
}
func ownMigrateConfig(p C.MigrateConfigRef) MigrateConfig {
	return MigrateConfig{
		driver: ownString(p.driver),
		dsn:    ownString(p.dsn),
	}
}
func cntMigrateConfig(s *MigrateConfig, cnt *uint) [0]C.MigrateConfigRef {
	_ = s
	_ = cnt
	return [0]C.MigrateConfigRef{}
}
func refMigrateConfig(p *MigrateConfig, buffer *[]byte) C.MigrateConfigRef {
	return C.MigrateConfigRef{
		driver: refString(&p.driver, buffer),
		dsn:    refString(&p.dsn, buffer),
	}
}
...
func newMigrateOutcome(p C.MigrateOutcomeRef) MigrateOutcome {
	return MigrateOutcome{
		version: newC_uint32_t(p.version),
		dirty:   newC_bool(p.dirty),
		error:   newString(p.error),
	}
}
func ownMigrateOutcome(p C.MigrateOutcomeRef) MigrateOutcome {
	return MigrateOutcome{
		version: newC_uint32_t(p.version),
		dirty:   newC_bool(p.dirty),
...
func cntMigrateOutcome(s *MigrateOutcome, cnt *uint) [0]C.MigrateOutcomeRef {
	_ = s
	_ = cnt
	return [0]C.MigrateOutcomeRef{}
}
func refMigrateOutcome(p *MigrateOutcome, buffer *[]byte) C.MigrateOutcomeRef {
	return C.MigrateOutcomeRef{
		version: refC_uint32_t(&p.version, buffer),
		dirty:   refC_bool(&p.dirty, buffer),
