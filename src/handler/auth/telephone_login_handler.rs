...
use crate::service::auth::telephone_login_service::TelephoneLoginService;
use result::http_result::{build_http_result, HttpResult};

pub async fn telephone_login(
    State(state): State<AppState>,
    Extension(device): Extension<DeviceContext>,
    Json(mut req): Json<TelephoneLoginRequest>,
) -> HttpResult {
    ...
    let svc = TelephoneLoginService::new(state.repos.clone(), state.config.clone(), state.cache.clone());
