...
use crate::service::auth::user_register_service::UserRegisterService;
use result::http_result::{build_http_result, HttpResult};

pub async fn user_register(
    State(state): State<AppState>,
    Extension(device): Extension<DeviceContext>,
    Json(mut req): Json<UserRegisterRequest>,
) -> HttpResult {
    ...
    let svc = UserRegisterService::new(state.repos.clone(), state.config.clone(), state.cache.clone());
