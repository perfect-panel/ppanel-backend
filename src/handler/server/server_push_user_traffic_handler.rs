/// Handler for POST /v1/server/push-traffic
/// Delegates to `server_push_user_traffic_service`.
...
use crate::service::server::server_push_user_traffic_service;
...
pub async fn server_push_user_traffic(
    ...
    headers: HeaderMap,
    Json(req): Json<ServerPushUserTrafficRequest>,
) -> HttpResult {
    ...
    let result = server_push_user_traffic_service::server_push_user_traffic(
        state.repos.clone(),
        state.config.clone(),
        req,
    )
