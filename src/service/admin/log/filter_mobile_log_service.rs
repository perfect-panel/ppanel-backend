use crate::model::dto::log::{FilterMobileLogResponse, MessageLog};
...
pub struct FilterMobileLogRequest {
    pub page: i32,
    pub size: i32,
...
pub async fn filter_mobile_log(
    repo: &dyn LogRepo,
    req: FilterMobileLogRequest,
) -> Result<FilterMobileLogResponse, anyhow::Error> {
    let (rows, total) = repo
        .filter_logs(
            req.page as i64,
            req.size as i64,
            Some(LogType::MOBILE_MESSAGE.0),
            req.date.as_deref(),
            None,
            req.search.as_deref(),
        )
    ...
    let list = rows.into_iter().map(message_log_from_row).collect();

    Ok(FilterMobileLogResponse { total, list })
}
