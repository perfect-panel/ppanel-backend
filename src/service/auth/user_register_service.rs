...
use crate::model::dto::auth::{LoginResponse, UserRegisterRequest};
...
impl UserRegisterService {
    ...
    pub async fn register(&self, req: UserRegisterRequest) -> Result<LoginResponse, anyhow::Error> {
        ...
        if self.config.email.enable_verify {
            let cache_key = format!("{}:1:{}", crate::config::cache_key::AUTH_CODE_CACHE_KEY, req.email);
        ...
        if let Some(u) = self.repos.user.find_one_by_email(&req.email).await
        ...
        self.repos.user.insert_auth_method(&AuthMethods {
            ...
            verified: self.config.email.enable_verify,
            created_at: now,
            updated_at: now,
        }).await
        .map_err(|e| anyhow!(CodeError::new_err_code_msg(error_code::DATABASE_INSERT_ERROR, e.to_string())))?;

        let trial_sub = if cfg.enable_trial {
        ...
        Telemetry::register(&self.repos, user.id, "email", &req.email, &req.ip, &req.user_agent).await;
    ...
    async fn activate_trial(&self, user_id: i64) -> Result<UserSubscribe, anyhow::Error> {
        ...
        let user_sub = UserSubscribe {
            ...
            token, uuid: Uuid::new_v4().to_string(), status: 1,
            note: String::new(),
